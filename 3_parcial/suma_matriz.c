...
#define ROW 10
#define COL 10
bool validar_suma_matriz(int filaA, int colA, int filaB, int colB);
void leer_datos_matriz(int matriz[ROW][COL], int fila, int col);
void imprimir_matriz(int matriz[ROW][COL], int fila, int col);
void suma_matriz(int matrizA[ROW][COL], int matrizB[ROW][COL], int matriz_resultante[ROW][COL], int fila, int col);
int main(void)
{
	int filaA, colA;
	int filaB, colB;
	...
	printf("Introduce la columna de la matriz B : ");
		scanf("%d", &colB);
	if (validar_suma_matriz( filaA, colA, filaB, colB))
		{
			...
			suma_matriz( matrizA,  matrizB,  matriz_resultante,  filaA,  colA);	
